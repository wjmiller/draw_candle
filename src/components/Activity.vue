<template lang="html">
  <div>
    ...
    <div>
      <button v-on:click="button1++" :disabled="!isActive">+</button>
      <span> {{ button1 }} </span>
      <button v-on:click="button1--" :disabled="!isActive">-</button>
    </div>
    <div>
      <button v-on:click="button2++" :disabled="!isActive">+</button>
      <span> {{ button2 }} </span>
      <button v-on:click="button2--" :disabled="!isActive">-</button>
    </div>
    <button v-on:click="makeAttempt" :disabled="!isActive">I Think I'm Right</button>
  </div>
</template>
