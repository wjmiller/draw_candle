...
export default {
  ...
  data () {
    return {
      ...
      levels: [
        {level: 1, ticks: [], active: false, targetPrice: 3.0, candleData: []},
        {level: 2, ticks: [], active: false, targetPrice: 1.0, candleData: []},
        {level: 3, ticks: [], active: false, targetPrice: 0.5, candleData: []},
        {level: 4, ticks: [], active: false, targetPrice: 0.2, candleData: []},
        {level: 5, ticks: [], active: false, targetPrice: 1.3, candleData: []}
      ]
    }
  },
  methods: {
  ...
  computed: {
    ...
    currentLevel() { return this.levels.find(level => this.level === level.level) },
    ticksLeft() { return Math.max(1, this.levelTimeEnd - this.levelTime) },
    priceDiff() { return this.targetPrice/this.price },
    avgBuyOrders() {return Math.ceil(this.avgOrdersPerTick/(this.priceDiff + 1) * this.priceDiff) },
