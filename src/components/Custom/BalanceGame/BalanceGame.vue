...
export default {
  ...
  data () {
    return {
      ...
      levels: [
        {level: 1, ticks: [], active: false, targetPrice: 3.0},
        {level: 2, ticks: [], active: false, targetPrice: 1.0},
        {level: 3, ticks: [], active: false, targetPrice: 0.5},
        {level: 4, ticks: [], active: false, targetPrice: 0.2},
        {level: 5, ticks: [], active: false, targetPrice: 1.3}
      ]
    }
  },
  methods: {
  ...
  computed: {
    displayPrice() { return (this.price/100).toFixed(2) },
    displayBuyOrders() { return `${this.orders.buy}` },
    displaySellOrders() { return `${this.orders.sell}` },
    maxLevels() { return this.levels.length },
    activeLevel() { return this.levels.find(level => level.active) },
    ticksLeft() { return Math.max(1, this.levelTimeEnd - this.levelTime) },
    priceDiff() { return this.targetPrice/this.price },
    avgBuyOrders() {return Math.floor(this.avgOrdersPerTick/(this.priceDiff + 1) * this.priceDiff) },
    avgSellOrders() { return this.avgOrdersPerTick - this.avgBuyOrders },
    tickBuyOrders() { return Math.floor(Math.random() * (this.avgBuyOrders + 1))},
    tickSellOrders() { return Math.floor(Math.random() * (this.avgSellOrders + 1))},
    previousTick() { return this.activeLevel ? this.activeLevel.ticks[this.activeLevel.ticks.length - 1] : null }
  },
  watch: {
    levelTime() {
      this.setTick()
