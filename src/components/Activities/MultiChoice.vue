...
export default {
    ...
    selectOption(option) {
      if (!this.completed) {
        ...
        if (!this.multipleSelect) {
          this.localOptions.filter(item => item.name !== option.name).forEach(item => item.selected = false)
        }
  ...
  computed: {
    hasFeedback() { return this.feedback && this.feedback.length >= 0 },
    selectedOptions() { return this.localOptions.filter(item => item.selected) },
    correctOptions() { return this.localOptions.filter(item => item.correct) },
    feedbackDisplayed() {
      ...
      this.localOptions.reduce((memo, item) => {
        ...
        return memo
      }, ['default', isCorrect, `default-${isCorrect}`]) :
      [`hint-${this.attempts}`]
    }
  },
  created() {
