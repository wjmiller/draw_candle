...
export default {
  ...
  data() {
    return {
      defaultMessages: [{name: 'default-correct', text: 'Correct'}, {name: 'default-incorrect', text: 'Incorrect'}],
      revealed: false
    }
  },
  methods: {
  ...
  computed: {
    allMessages() { return [].concat(this.messages.length > 0 ? this.messages : this.defaultMessages) },
    displayOn() { return this.activated && (this.auto || this.revealed) },
    displayedMessages() { return this.displayed.reduce((memo, item) => {
